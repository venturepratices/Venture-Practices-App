...
/**
 * Left sidebar for the Planning tab's "Ideas" view — "All ideas" plus the
 * client's own folders, mirroring AssetFolderSidebar's URL-driven selection
 * (?folderId=...) so it composes cleanly with the tab (?tab=) and filter
 * params already on the URL. Simpler than AssetFolderSidebar: no mobile
 * drawer overlay — it just stacks above the list on small screens.
 */
export function PlanningFolderSidebar({
  clientId,
  folders,
  allCount,
  canManage,
  ...
  async function createFolder() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/clients/${clientId}/planning-folders`, {
        method: "POST",
  ...
  async function submitRename() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/planning-folders/${renamingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteFolder(folder: FolderItem) {
    if (!window.confirm(`Delete the "${folder.name}" folder? Its ideas will move to "All ideas" — nothing is deleted.`)) return;
    setBusy(true);
    try {
      await fetch(`/api/planning-folders/${folder.id}`, { method: "DELETE" });
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-0.5 md:w-[200px]">
      <Link
        href={hrefFor(null)}
        className={cn(
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">All ideas</span>
        <span className="text-xs text-muted-foreground">{allCount}</span>
      </Link>

      {folders.length > 0 ? <div className="my-2 h-px bg-border" /> : null}
